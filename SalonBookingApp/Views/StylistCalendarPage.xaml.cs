...
namespace SalonBookingApp.Views
{
    public partial class StylistCalendarPage : ContentPage
    {
        ...
        public StylistCalendarPage()
        {
            ...
            if (App.StilistLogat != null) _idStilistSelectat = App.StilistLogat.ID;
        ...
        {
            var luni = DateTimeFormatInfo.CurrentInfo.MonthNames.Where(m => !string.IsNullOrEmpty(m)).Select(m => char.ToUpper(m[0]) + m.Substring(1)).ToList();
        ...
        private async Task IncarcaStilisti()
        {
            try
            {
                ...
                var listaOrdonata = App.EsteAdmin ? totiStilistii.OrderBy(s => s.FirstName).ToList() : totiStilistii.OrderByDescending(s => s.ID == App.StilistLogat?.ID).ThenBy(s => s.FirstName).ToList();
                _stilisti.Clear();
                foreach (var s in listaOrdonata)
                {
                    _stilisti.Add(new ModelStilistSelectabil { Stilist = s, IsSelected = s.ID == _idStilistSelectat, PozaPath = s.FirstName.ToLower() + ".png" });
                }
            }
        ...
        private void GenereazaZilePentruLuna(DateTime dataDinLuna)
        {
            ...
            for (int i = 1; i <= zileInLuna; i++)
            {
                ...
                _zileleLunii.Add(new ModelZiCalendar { DataCompleta = dataCurenta, NumeZi = dataCurenta.ToString("dddd").Substring(0, 1).ToUpper(), NumarZi = i.ToString(), IsSelected = dataCurenta.Date == _dataSelectata.Date });
            }
        ...
        private async void FaScrollLaZiuaCurenta()
        {
            ...
            MainThread.BeginInvokeOnMainThread(() => { var ziCurenta = _zileleLunii.FirstOrDefault(z => z.IsSelected); if (ziCurenta != null) ListaZile.ScrollTo(ziCurenta, position: ScrollToPosition.Start, animate: false); });
        }
        ...
        private async void IncarcaProgramari(DateTime data)
        {
            ...
            try
            {
                ...
                var programariZi = toateProgramarile.Where(p => p.StylistID == _idStilistSelectat && p.AppointmentDate.ToLocalTime().Date == data.Date).OrderBy(p => p.AppointmentDate).ToList();
                ...
                foreach (var p in programariZi)
                {
                    ...
                    if (client != null) { p.ClientNameDisplay = $"{client.FirstName} {client.LastName}"; p.Client = client; }
                }
        ...
        private void DeseneazaTimeline(List<Appointment> programari)
        {
            ...
            for (int i = 0; i < totalHours; i++)
            {
                ...
                var timeLabel = new Label { Text = $"{_startHour + i:D2}:00", TextColor = Color.FromArgb("#8E8E93"), FontSize = 14, FontAttributes = FontAttributes.Bold, VerticalOptions = LayoutOptions.Start, Margin = new Thickness(0, 0, 10, 0), HorizontalTextAlignment = TextAlignment.End };
            ...
            foreach (var p in programari)
            {
                ...
                var card = new Border { BackgroundColor = isPast ? Color.FromArgb("#F2F2F2") : Color.FromArgb("#FFF0F2"), StrokeThickness = 0, Padding = new Thickness(5, 5), StrokeShape = new RoundRectangle { CornerRadius = 10 } };
                ...
                vStack.Children.Add(new Label { Text = p.ClientNameDisplay, FontSize = 15, FontAttributes = FontAttributes.Bold, TextColor = isPast ? Color.FromArgb("#555555") : Colors.Black });
                if (p.Service.Duration >= 45) vStack.Children.Add(new Label { Text = p.Service.DisplayName, FontSize = 13, TextColor = isPast ? Color.FromArgb("#888888") : Color.FromArgb("#444444") });
                vStack.Children.Add(new Label { Text = $"{p.AppointmentDate:HH:mm}", FontSize = 12, TextColor = isPast ? Color.FromArgb("#666666") : Color.FromArgb("#D28A99"), FontAttributes = FontAttributes.Bold });

                if (!isPast)
                {
                    var btnAnuleaza = new Label { Text = "Anulează", TextColor = Colors.Red, FontSize = 11, FontAttributes = FontAttributes.Bold, HorizontalOptions = LayoutOptions.End, Margin = new Thickness(0, -15, 5, 0) };
                    var tapGesture = new TapGestureRecognizer();
                    tapGesture.CommandParameter = p;
                    tapGesture.Tapped += OnCancelByStylistTapped;
                    btnAnuleaza.GestureRecognizers.Add(tapGesture);
                    vStack.Children.Add(btnAnuleaza);
                }

                gridCard.Children.Add(dungaAccent);
                gridCard.Children.Add(vStack);
        ...
        private async void OnCancelByStylistTapped(object sender, TappedEventArgs e)
        {
            var programare = e.Parameter as Appointment;
            if (programare == null) return;
            var popup = new ConfirmPopup("Anulare Programare", "Ești sigur că vrei să anulezi această programare?");
            ...
            if (popup.IsConfirmed)
            {
                ...
                try
                {
                    ...
                    var payload = new
                    {
                        action = "stylist_cancel",
                        client_email = programare.Client?.Email ?? "andaanduta18@yahoo.ro",
                        client_name = programare.Client?.FirstName ?? "Client",
                        message_details = $"Stilistul {App.StilistLogat?.FirstName} a anulat programarea din data de {programare.AppointmentDate:dd.MM.yyyy} la ora {programare.AppointmentDate:HH:mm} pentru serviciul {programare.Service?.DisplayName}.",
