...
public partial class ReviewAppointmentPage : ContentPage
{
    ...
    {
        InitializeComponent();
        _slot = slot; _stylist = stylist; _service = service; _pachetActiv = pachetActiv; _packageSlotIndex = packageSlotIndex; _esteAchizitiePachet = false;
    ...
    {
        InitializeComponent();
        _pachetNou = pachetNou; _pretPachetNou = pretPachet; _esteAchizitiePachet = true;
    ...
    private void IncarcaDateProgramare()
    {
        ...
        LabelDataOra.Text = $"{_slot.FullDateTime.ToString("dd MMMM yyyy", culture)}, ora {_slot.FullDateTime.ToString("HH:mm")}";
        ...
        if (_pachetActiv != null) { LabelTotal.Text = "0 lei"; LabelMetodaPlataTitlu.Text = AppResources.PackageAppliedTitle; LabelMetodaPlataDescriere.Text = AppResources.PackageAppliedDesc; }
        else LabelTotal.Text = $"{_service.Price} lei";
    }

    private void IncarcaDateAbonament()
    {
        GridData.IsVisible = false; Separator1.IsVisible = false;
        TitluPaginaHeader.Text = _pachetNou.PackageName; TitluCard.Text = _pachetNou.PackageName;
        LabelServiciu.Text = AppResources.IncludedServices;
        LabelDurataPret.Text = _pachetNou.PackageType == "Sedinte" ? $"{_pachetNou.TotalUses} x {AppResources.LabelChooseService}" : "Abonament VIP Lunar";
        LabelMetodaPlataTitlu.Text = AppResources.PaymentLocationTitle; LabelMetodaPlataDescriere.Text = AppResources.SubPaymentNote;
        LabelTotal.Text = $"{_pretPachetNou} lei"; BtnConfirmare.Text = AppResources.ConfirmSubBtn;
    }
    ...
    private async void OnConfirmClicked(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                var appt = new Appointment { AppointmentDate = _slot.FullDateTime, StylistID = _stylist.ID, ServiceID = _service.ID, ClientID = App.UserLogat?.ID ?? 0 };
                await App.Database.SaveAppointmentAsync(appt);
                if (_pachetActiv != null)
                {
                    if (_pachetActiv.PackageType == "Sedinte") _pachetActiv.RemainingUses--;
                    else if (_packageSlotIndex == 1) _pachetActiv.RemainingService1--;
                ...
                try
                {
                    ...
                    var payload = new
                    {
                        ...
                        date = _slot.FullDateTime.ToString("dd.MM.yyyy"),
                        time = _slot.FullDateTime.ToString("HH:mm"),
                        service_name = _service.DisplayName,
                        is_package = (_pachetActiv != null)
                    };
                ...
                try
                {
                    var request = new NotificationRequest { NotificationId = 1000 + appt.ID, Title = AppResources.SuccessTitle, Description = $"{AppResources.BookNow}: {appt.AppointmentDate:HH:mm}", Schedule = new NotificationRequestSchedule { NotifyTime = DateTime.Now.AddSeconds(1) } };
        ...
        catch (Exception ex) { await ArataNotificareRoz(AppResources.ErrorTitle, true); }
    }
